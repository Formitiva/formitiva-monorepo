...
vi.mock('react', () => ({
  ...
  createContext: <T>(defaultValue: T) => ({ Provider: null, Consumer: null, _currentValue: defaultValue }),
  useContext: () => undefined,
  useState: <T>(init: T) => [init, () => undefined],
  useReducer: (_reducer: unknown, init: unknown) => [init, () => undefined],
...
describe('registerComponent / getComponent', () => {
  it('registers and retrieves a custom component', () => {
    const myComp = () => null;
    registerComponent('custom-react-widget', myComp);
    expect(getComponent('custom-react-widget')).toBe(myComp);
  });

  it('returns undefined for an unregistered type', () => {
    expect(getComponent('nonexistent-type-react-xyz')).toBeUndefined();
  });
...
describe('pluginRegistry', () => {
  afterEach(() => {
    unregisterPlugin('test-react-plugin-a');
    unregisterPlugin('test-react-plugin-b');
  });

  it('registers a plugin and detects it with hasPlugin', () => {
    registerPlugin({ name: 'test-react-plugin-a', version: '1.0.0' });
    expect(hasPlugin('test-react-plugin-a')).toBe(true);
  });

  it('retrieves a registered plugin with getPlugin', () => {
    registerPlugin({ name: 'test-react-plugin-a', version: '2.0.0', description: 'Desc' });
    const p = getPlugin('test-react-plugin-a');
    expect(p?.name).toBe('test-react-plugin-a');
  ...
  it('getAllPlugins includes all registered plugins', () => {
    registerPlugin({ name: 'test-react-plugin-a', version: '1.0.0' });
    registerPlugin({ name: 'test-react-plugin-b', version: '1.0.0' });
    const names = getAllPlugins().map((p) => p.name);
    expect(names).toContain('test-react-plugin-a');
    expect(names).toContain('test-react-plugin-b');
  });

  it('hasPlugin returns false for an unregistered plugin', () => {
    expect(hasPlugin('nonexistent-plugin-react-xyz')).toBe(false);
  });

  it('unregisterPlugin removes the plugin and returns true', () => {
    registerPlugin({ name: 'test-react-plugin-a', version: '1.0.0' });
    expect(unregisterPlugin('test-react-plugin-a')).toBe(true);
    expect(hasPlugin('test-react-plugin-a')).toBe(false);
  });

  it('unregisterPlugin returns false for a non-existent plugin', () => {
    expect(unregisterPlugin('does-not-exist-react-xyz')).toBe(false);
  });

  it('calls setup on registration', () => {
    const setup = vi.fn();
    registerPlugin({ name: 'test-react-plugin-a', version: '1.0.0', setup });
    expect(setup).toHaveBeenCalledOnce();
  });

  it('calls cleanup on unregistration', () => {
    const cleanup = vi.fn();
    registerPlugin({ name: 'test-react-plugin-a', version: '1.0.0', cleanup });
    unregisterPlugin('test-react-plugin-a');
  ...
  it('throws on duplicate registration with conflictResolution "error"', () => {
    ...
    expect(() =>
      registerPlugin(
        { name: 'test-react-plugin-a', version: '2.0.0' },
  ...
  it('skips duplicate registration with conflictResolution "skip"', () => {
    ...
    expect(getPlugin('test-react-plugin-a')?.version).toBe('1.0.0');
  });
});
